...
### 더 나은 방법이 있으면 반드시 제안한다

**OTA로 보낼 수 있는지, 코드 수정 범위가 얼마나 커지는지를 이유로 열등한 방법을
택하지 않는다.** 성능·안정성·유지보수성에서 더 나은 구현 기술이나 방식이 있다면
그것을 **먼저 제안한다.** 스토어 빌드가 필요하든, 네이티브 모듈이 늘든,
파일을 여러 개 고쳐야 하든 그것은 제안을 막는 이유가 아니다.

제안할 때 함께 밝힌다:
- 왜 그 방법이 더 나은가 (구체적으로 — "표준이라서"는 이유가 아니다)
- 무엇을 치러야 하는가 (스토어 빌드, 런타임 버전, 의존성, 작업량)
- 그 방법으로 **새로 생길 수 있는 문제**

값싼 우회로 두세 번 고치다 결국 제대로 만들게 되는 것이 가장 비싸다.
실제로 로그인 화면 키보드 문제를 그렇게 세 번 고쳤다(2026-09-06~07).

판단은 사용자가 한다. 선택지를 감추지 않는다.

### 추측하지 말고 측정한다

같은 기간에 원인을 세 번 틀렸다. 매번 코드를 읽고 "이것 때문일 것"이라고
단정했고, 실제로 재보니 아니었다:

- 웹 오른쪽 흰 띠 → `viewport-fit=cover`인 줄 알았으나 장식 원의 오버플로였다
  (DevTools로 `docScrollWidth 553 / clientWidth 502` 측정)
- 복구 세션 판별 → `amr` 클레임을 실제로 찍어보고서야 `otp`인 것을 알았다
- 키보드 가림 → `ScrollView`만 씌우면 될 줄 알았으나 `flex:1`이 흡수해 스크롤이
  생기지 않았고, 진짜 원인은 `space-between`이 카드를 바닥에 고정한 것이었다
...
### 흐름(계약) 테스트 — `__tests__/flows/`

**한 레이어만 보는 테스트는 이 앱에서 가장 비싼 버그를 못 잡는다.** "일정 제안 →
사용자 수락"이 서로 독립된 네 가지 이유로 동시에 죽은 채 배포돼 있었는데
(쓰는 쪽 상태 ≠ 읽는 쪽 조건, Date가 ISO로 저장, 수락 RPC 부재, DB 가드 차단)
당시 테스트 146개 중 하나도 울리지 않았다. 전부 **레이어 경계의 계약** 문제라
각각을 따로 보면 멀쩡해 보였기 때문이다.

그래서 기능이 **화면 → 서비스 → DB**를 가로지르면 흐름 테스트를 함께 쓴다.

- 네트워크 경계(`supabase`)만 막고 **실제 코드**를 흐름 순서대로 통과시킨다
- 한 단계의 **출력을 다음 단계의 입력으로 그대로** 넣어 계약을 확인한다
  (저장 payload를 잡아 그것을 읽는 쪽 헬퍼에 그대로 먹인다)
- 앱의 허용 조건이 **DB 가드·RPC가 허용하는 것과 같은지** 단언한다.
  넓으면 사용자가 원인 불명 실패를 겪고, 좁으면 되는 걸 못 하게 된다
- 본보기: `__tests__/flows/consultationScheduleFlow.test.js`

**테스트를 쓴 뒤 버그를 일부러 되돌려 실패하는지 확인한다.** 통과만으로는 그
테스트에 힘이 있는지 알 수 없다 — 위 파일은 원래 버그 2종을 재주입해
해당 4건만 실패하는 것을 확인했다.

상태 규칙을 바꾸면 **앱 상수 · DB 가드 · 테스트 셋을 같이** 바꾼다
(`src/constants/consultation.js` 주석 참고).
