...
- Supabase 대시보드 설정(Auth/Google provider·RLS·마이그레이션)은 **반드시 앱이 쓰는 프로젝트 `thorgkxpbhsttgskhepu`**(`src/lib/supabase.js:16`)에서 한다. 다른 Supabase 프로젝트에 설정하면 앱엔 반영되지 않으니, 대시보드에서 ref를 꼭 확인할 것.
