...
## 충실도 향상 3: 마이페이지 시안 구현 (.dc.html pos 84502)
사용자 제공 마이페이지 시안에 맞춰 MyPageScreen 재구성.
- 타이틀바("마이페이지" 17/800 중앙) + 프로필 카드(60 아바타+이름18/800+이메일·전화) +
  **통계 카드 3분할**(구매상담/판매상담/내차량 카운트, 22/800 블루 + 구분선) +
  **세그먼트 탭**(구매/판매/내차량, 좌측정렬 2px 밑줄 인디케이터) +
  로그아웃(아웃라인 ghost) + 회원탈퇴(회색 텍스트 링크).
- 2-5에서 분리했던 상담 TabView를 시안대로 마이페이지에 복원(상담 전용 탭과 공존 — 시안도 동일).
- 중첩 가상스크롤 회피: 상단 고정 + 리스트 내부 스크롤(TabView flex:1) + 하단 액션 고정.
- 상담 카드(Buy/Sell 탭): 썸네일(50, 라운드12) + 이름(15/800) + 날짜·시간 + 상태칩 가로 레이아웃.
- 검증: lint 0 error, Metro 번들 성공.

## 충실도 향상 4: 내 차량 = 차량카드 C(미니멀) + 그림자 보정
- MyVehiclesTab: 시안 차량카드 **C(미니멀 텍스트)** 적용 — #F8F9FA 컨테이너(라운드20, 그림자 없음)
  안에 행 리스트(썸네일56/라운드14 + 이름16/800 + 작은 차종태그10px + 제조사·연식·주행 12/회색 +
  우측 가격 17/800 블루 + "만원"11). 행 사이 #ECEEF1 구분선. .dc.html pos 52293 기준.
- 그림자 버그 수정 2건:
  1. shadows.soft: radius 30→12, offset 8→5, opacity .07→.10 (큰 radius가 모서리 광배를 만들던 문제).
  2. VehicleCard: overflow:hidden+shadow를 같은 View에 줘 iOS에서 그림자가 잘리던 문제 →
     그림자 호스트(바깥)와 이미지 클립(안쪽 래퍼) 분리.
- 검증: lint 0 error, Metro 번들 성공.

## 충실도 향상 5: 메인 홈 시안 A안 2섹션 + 가격 숨김
- 가격 정책: 홈 공개 브라우즈에서 **일반 사용자에게 가격 미노출**(hidePrice = role!=='admin').
  기존엔 소유자가 본인 차 가격을 봤으나, 홈에선 전원 "상담 후 안내".
- 시안 A안 2섹션:
  - "새로 등록된 차량"(createdAt 최근순, 가로 스크롤, 220폭 카드)
  - "이번 주 인기 차량"(컴팩트 행: 썸네일70x52 + 이름 + 제조사·연식 + 가격숨김)
