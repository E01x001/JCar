...
## [ISSUE-09] "실사진 1장 이상" 노출 규칙이 지금은 걸러내지 않는다 🟢

**발견**: 2026-09-12 · **상태**: 의도된 상태로 유지(사용자 결정) · **영향**: 회귀 방지

`20260823133000_require_real_photo.sql`은 실사진 없는 차량을 노출하지 않으려고
노출 조건에 `array_length(image_urls, 1) >= 1`을 더했고, 주석에
"카탈로그 이미지로 노출이 열리면 이 규칙을 만든 이유가 사라진다"고 적어 뒀다.

**지금 노출 중인 14대는 전부 그 상태다.** 실측(2026-09-12):

```
버킷 파일을 가리키는 칸: 0      카탈로그(cartory.net) 칸: 14
image_urls[1] = catalog_image_url 인 차량: 14 / 14
```

옛 Firestore에서 이식한 차량을 목록에 띄우려고 카탈로그 주소를 `image_urls`에도
복제해 넣었기 때문이다.

**고치지 않는다.** 이 차량들은 팔려고 올린 매물이 아니라 **계약 자리에서 앱이
어떻게 동작하는지 보여주려고 등록해 둔 시연용 데이터**다. 노출 조건에서 복제된
카탈로그를 빼면 보여줄 매물이 0대가 된다.

그래서 이 항목은 "고쳐야 할 것"이 아니라 **다시 고치려 드는 것을 막는 기록**이다.
실제 매물이 올라오기 시작하고 시연용 차량을 내릴 때가 규칙을 되살릴 시점이다.

다만 화면 쪽은 고쳤다 — 배열에 있다고 실사진으로 세면 흰 배경 PNG를 `cover`로
잘라 그리게 되므로, `utils/vehicleImage.js`의 `realPhotosOf`가 카탈로그와 같은
주소를 걸러낸다. `MyVehiclesTab`의 "사진 없음" 안내는 **노출 여부**를 말하는
문구라 그대로 둔다 — 노출 조건이 배열 길이만 보는 지금은 그 표현이 맞다.
