...
### 남은 것 — 이게 없으면 여전히 안 된다

**커스텀 SMTP가 없다.** `smtp_host: null`이라 Supabase 내장 메일을 쓰고 있고,
프로젝트 전체 **시간당 2통**이다(가입 확인 메일과 공유). Supabase가 개발용이라고
명시한 경로다. 실사용에서는 재설정 메일이 대부분 발송되지 않는다.

→ Resend·SendGrid 등에서 자격증명을 받아 Auth 설정에 넣어야 한다. 코드는 이미
완성돼 있어 SMTP만 붙이면 동작한다.

곁가지: `password_min_length`가 6이다. 앱은 8자를 요구하지만 서버 기준은 6이므로,
서버 값도 8로 올려두는 편이 낫다.
